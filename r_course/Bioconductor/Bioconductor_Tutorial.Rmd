...
## Basic classes of BioC packages {.smaller}
- General infrastructure
    - Biobase, IRanges, GenomicRanges, Biostrings, BSgenome, GenomicFeatures
- Annotation
    - annotate, biomaRt, 
- Visualisation
    - geneplotter, ggbio, Gviz
- Transcription Factor motif search
    - PWMEnrich, MotifDb, 
- Miroarray analysis
    - affy, affycomp, limma,
- NGS Data analysis
    - ShortRead, Rsamtools, GenomicRanges, GenomicAlignments, rtrackalyer
- Graphs and Networks
    - graph, RBGL, Rgraphviz
- Flow Cytometry
    - prada, flowCore, flowViz, flowUtils
- Proteomics
    - RforProteomics, mzR, TargetSearch, MSnbase
- Protein Interactions
    - ppiData, ppiStats, ScISI, Rintact


## BioC packages for NGS Data Analysis{#highlight1 .smaller}
...
- Builds on top of Interval Ranges (IRanges) package and lays foundation for NGS analysis. 
...
- One of the common tasks in NGS data analysis
...
## Computing Coverage
<b>coverage</b> calculates how many ranges overlap individual positions in the genome. <b>coverage</b> function returns the coverage as Rle instance.
```{r, echo=TRUE, eval=TRUE, message=FALSE}
coverage(gr1)
```
...
Header: Meta data (reference genome, aligner), starts with “@”
...
- GenomicRanges package defines the GAlignments class – a specialised class for storing set of genomic alignments (ex: NGS data) 
...
- The readGAlignmentsFromBam function takes an additional argument, <b>param</b> allowing the user to customise which genomic regions and which fields to read from BAM
-<b>param</b> can be constructed using </b>ScanBamParam</b> function

```{r, echo=TRUE, eval=TRUE, message=FALSE}
library(GenomicAlignments)
SampleAlign <- readGAlignmentsFromBam(BamFile)
```
...
SampleAlign1 <- readGAlignmentsFromBam(BamFile,param=param1)
```
