...
```{r,message=T}
 catAndExpr <- read.delim("categoriesAndExpression.txt")
```
...
```{r,message=T}
 lmExercise <- read.delim("lmExercise.txt")
```
...
- Predict values for Y and Z when X is 100
```{r,message=T}
str(lmXY)
predictedY <- unname(lmXY$coefficients[1] + lmXY$coefficients[2]*100)
predictedZ <- unname(lmXZ$coefficients[1] + lmXZ$coefficients[2]*100)
predictedY
predictedZ
```
